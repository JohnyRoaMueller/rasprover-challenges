...
# Camera funcs
cvf = cv_ctrl.OpencvFuncs(thisPath, base) # init of the cam
...
@app.route('/video_feed/challange_1')
def video_feed_challange_1():
    from challange_1 import generate_frames_challange_1
    return Response(generate_frames_challange_1(cvf), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/video_feed/challange_2')
def video_feed_challange_2():
    return Response(generate_frames(), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/video_feed/challange_3')
def video_feed_challange_3():
    return Response(generate_frames(), mimetype='multipart/x-mixed-replace; boundary=frame')
...
def joystick_loop():
    while True:
        pygame.event.pump()  # Wichtig: Events updaten
        if joystick.get_button(0):  # Button 0 gedrückt
            print("Challenge 1 starten")
            # Hier z.B. Thread starten oder Status setzen
            #threading.Thread(target=run_challange_1, daemon=True).start()
...
if __name__ == "__main__":
    ...
    #run hotcode
    #from hotloader import reload_on_change
